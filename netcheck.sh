...
PRINT_SPEEDTEST() {
  echo "intermittent speed test results $(date)" >> $VAR_LOGFILE
  echo -e $COLOR_GREEN"intermittent speed test results $(date)"$COLOR_RESET
}
  ...
  while true; do
    ...
    if [[ $? -eq 0 ]]; then :
      ...
      if [[ $VAR_CONNECTED = false ]]; then :
        ...
        # We are online, run a speed test every 30 minutes
        # var_speedtest_interval=0
        ...
        if (( $var_speedtest_interval > 100 )); then
          ...
          # var_speedtest_interval=$(($SECONDS / 1))
          # echo "var_speedtest_interval: $var_speedtest_interval"
          sleep 2
        fi
      fi
